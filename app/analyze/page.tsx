          ...
          {data && (
            <div className="space-y-6 animate-fade-in transition-all duration-500 opacity-100">
              <div className="px-8 py-6">
                ...
                {/* NEW BLOCK — ISSUES BY CATEGORY */}
                {data?.issues_by_category && (
                  <div className="mt-10">
                    <h3 className={styles.titleSection}>UX Issues by Category</h3>
                    <IssuesByCategory issues={data.issues_by_category} />
                  </div>
                )}

                {/* NEW BLOCK — COPYWRITING */}
                {data?.copywriting && (
                  <div className="mt-10">
                    <h3 className={styles.titleSection}>Copywriting Improvements</h3>
                    <CopywritingBlock items={data.copywriting} />
                  </div>
                )}
...
// -----------------------------------------------------
// COMPONENT: IssuesByCategory
// -----------------------------------------------------
function IssuesByCategory({ issues }: { issues: any }) {
  const categories = ["Clarity", "Navigation", "Visuals", "Trust", "Conversion"];

  return (
    <div className="space-y-8">
      {categories.map((cat) => {
        const block = issues?.[cat];
        if (!block) return null;

        return (
          <div
            key={cat}
            className="rounded-xl border border-[var(--stroke-light)] bg-white p-5"
          >
            {/* CATEGORY HEADER */}
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-[#061C2F]">{cat}</h3>
              <span className="text-sm font-medium text-[#375BE7]">
                {block.score}% now
              </span>
            </div>
            ...
            <div className="space-y-6">
              {block.issues?.map((issue: any, idx: number) => (
                <div key={idx} className="space-y-2 pb-4 border-b last:border-none border-[var(--stroke-light)]">
                  {/* DESCRIPTION */}
                  <p className="text-[15px] font-medium text-[#061C2F]">
                    {issue.description}
                  </p>
...
// -----------------------------------------------------
// COMPONENT: CopywritingBlock
// -----------------------------------------------------
function CopywritingBlock({ items }: { items: any[] }) {
  return (
    <div className="rounded-xl border border-[var(--stroke-light)] bg-white p-5 space-y-6">
      {items.map((item, idx) => (
        <div
          key={idx}
          className="space-y-4 pb-5 border-b last:border-none border-[var(--stroke-light)]"
        >
          {/* ELEMENT NAME */}
          <div className="text-sm font-semibold text-[#375BE7]">
            {item.element}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {/* BEFORE */}
            <div>
              <div className="text-xs font-medium text-[#8E99A2] mb-1">Before</div>
              <div className="rounded-lg bg-[var(--gray-soft)] p-3 text-sm text-[#061C2F]">
                {item.before}
              </div>
            </div>
            ...
            {/* AFTER */}
            <div>
              <div className="text-xs font-medium text-[#8E99A2] mb-1">After</div>
              <div className="rounded-lg bg-[#EEF2FF] border border-[#DCE2E7] p-3 text-sm text-[#061C2F]">
                {item.after}
              </div>
            </div>
          </div>
