...
export function ReportTrustMetaPanel({ meta }: Props) {
  const trustNotes = (meta.trust_notes ?? []).filter(Boolean);
  const heading = meta.proof_suggestion ?? "Trust gaps to address";
  ...
  return (
    <section className="overflow-hidden rounded-[16px] border border-v2-card-border bg-v2-card shadow-[0_1px_2px_rgba(27,26,23,0.03)]">
      <div className="flex items-center gap-2.5 border-b border-v2-card-divider px-6 py-[18px]">
        ...
        {trustNotes.length > 0 && (
          <span className="font-mono ml-auto text-[11.5px] tracking-[0.04em] text-v2-ink-faint">
            {trustNotes.length} GAP{trustNotes.length !== 1 ? "S" : ""}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 divide-y divide-v2-card-divider sm:grid-cols-2 sm:divide-x sm:divide-y-0">
        {/* Trust gaps */}
        <div className="p-6">
          <p className="mb-4 text-[15px] font-bold text-v2-ink">{heading}</p>
          {trustNotes.length > 0 ? (
            <div className="flex flex-col gap-2.5">
              {trustNotes.map((note, i) => (
                <div key={i} className="flex items-start gap-2.5 py-1">
                  <RiErrorWarningLine size={16} className="mt-px shrink-0 text-v2-critical-text" />
                  <span className="text-[15px] leading-[1.5] text-v2-dark-alt">{note}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-[15px] leading-[1.5] text-v2-ink-muted">No trust gaps identified.</p>
          )}
        </div>
