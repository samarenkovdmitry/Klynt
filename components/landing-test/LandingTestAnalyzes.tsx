  ...
  return (
    <section className={`${UPDATE_SECTION} bg-white`}>
      <div className={LANDING_UPDATE_CONTAINER}>
        ...
        <div className={`${UPDATE_SECTION_GRID} grid grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-4 md:gap-8`}>
          {ANALYZE_ITEMS.map((item) => {
            const Icon = item.icon;

            return (
              <div key={item.title} className="text-center">
                <div className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-[#2563EB] text-white">
                  <Icon size={20} />
                </div>
