  ...
  if (id === "prioritized-fixes") {
    return (
      <div className="w-[180px] space-y-2">
        {[
          ["Hero clarity", "bg-[#F87171]", "max-w-[120px]"],
          ["CTA hierarchy", "bg-[#FB923C]", "max-w-[100px]"],
          ["Trust signals", "bg-[#94A3B8]", "max-w-[140px]"],
          ...
          <div key={label} className="flex items-center gap-2">
            ...
            <span className="w-20 shrink-0 text-right text-[10px] text-[#6B7280]">
              {label}
            </span>
          </div>
        ))}
  ...
  return (
    <section className={`${UPDATE_SECTION} bg-white`}>
      <div className={LANDING_UPDATE_CONTAINER}>
        ...
        <div className={`${UPDATE_SECTION_GRID} grid grid-cols-1 gap-5 md:grid-cols-2`}>
          {WHAT_YOU_GET_FEATURES.map((feature) => {
            ...
            return (
              <article
                key={feature.id}
                className={`grid grid-cols-1 items-center gap-6 overflow-hidden rounded-[24px] border border-[rgba(6,28,47,0.06)] p-8 sm:grid-cols-[1fr_auto] ${cardPattern}`}
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[22px] font-semibold tracking-[-0.03em] text-[#061C2F]">
                      {feature.title}
                    </h3>
                  </div>
