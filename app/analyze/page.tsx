...
async function handleAnalyze() {
  ...
  function clarityToLabel(value: number) {
  if (value <= 3) return "Low";
  if (value <= 7) return "Medium";
  return "High";
}

function getPrimaryIssueColor(issue: string) {
  if (!issue) return "text-gray-600";

  const normalized = issue.toLowerCase();

  if (normalized.includes("clarity")) return "text-[#B91C1C]"; // red
  if (normalized.includes("hierarchy")) return "text-[#B45309]"; // amber
  if (normalized.includes("trust")) return "text-[#1D4ED8]"; // blue
  if (normalized.includes("cta")) return "text-[#4338CA]"; // indigo
  if (normalized.includes("conversion")) return "text-[#15803D]"; // green
...
function generateImpact(severity: string) {
  if (severity === "high") {
    return { clarity: 12, cta: 8 };
  }
  if (severity === "medium") {
    return { clarity: 6, cta: 4 };
  }
  return { clarity: 2, cta: 1 };
}

function getImpactColor(severity: string) {
  if (severity === "high") return "#FF383C";     // красный
  if (severity === "medium") return "#EA7B03";   // оранжевый
  return "#6B7280";                              // серый (low)
}
...
/>
  ...
  className={`
    ...
    ${loading
      ...
      : "hover:border-[#8E99A2] cursor-pointer"
    }
  `}
  style={{
    mask: "linear-gradient(#fff 0 0) padding-box, repeating-linear-gradient(90deg, #000 0 4px, transparent 4px 4px) border-box",
    WebkitMask: "linear-gradient(#fff 0 0) padding-box, repeating-linear-gradient(90deg, #000 0 4px, transparent 4px 4px) border-box",
  }}
...
/>
  ...
  <p className="text-xs text-[var(--ink-secondary)]">
    Max 20 MB. Use full-page screenshot for best results.
  </p>
</div>
        ...
        <div className="mt-8 w-full">
          {!loading ? (
            <Button disabled={isButtonDisabled} onClick={handleAnalyze}>
  Analyze UX
</Button>
    ...
    Generated by AI
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className={styles.card}>
            ...
            <p className={`${styles.caption} mt-auto`}>
              Overall UX quality
            </p>
          </div>
...
</div> {/* end suggested improvements */}
...
<div className="mb-6 space-y-4">
  ...
  <div className="rounded-xl bg-white border border-[var(--stroke-light)] p-5 space-y-5">
